      ...
      if (catArr.length > 0) {
        try {
          let rootCatArr = await Promise.all(catArr.map(async (cat) => {
            let _subNavMenu = new NavItemModel(cat.id.toString(), cat.name, cat.name.toLowerCase());

            //set as mega menu for video category
            _subNavMenu.megaMenu = cat.id.toString() == '2';

            let subCat = await this._categoryService.getSubCategoriesById(cat.id, 0, 4);


            if (subCat.length > 0) {
              _subNavMenu.navItems = await Promise.all(subCat.map(async (subCatItem) => {
                try {
                  let subCatVideo: VideoModel[] = await this._videoService.getVideoByCategoryId([subCatItem.id], 0, 4);

                  if (_subNavMenu.megaMenu) {
                    let megaMenuItem: NavItemModel = new NavItemModel(
                      subCatItem.id.toString(),
                      subCatItem.name,
                      cat.name.toLowerCase() + '/category/' + btoa(subCatItem.id.toString())
                    );
